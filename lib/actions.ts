...
export async function addBill(fd:FormData){const uid=await userId(); await prisma.recurringBill.create({data:{userId:uid,accountId:String(fd.get('accountId')),name:String(fd.get('name')),amount:toNumber(fd.get('amount')),dueDay:Number(fd.get('dueDay')||1),notes:String(fd.get('notes')||'')}}); revalidatePath('/bills'); revalidatePath('/dashboard');}
