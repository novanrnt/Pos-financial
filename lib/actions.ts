...
export async function deleteAccount(fd:FormData){const uid=await userId(); await prisma.account.deleteMany({where:{id:String(fd.get('id')),userId:uid}}); revalidatePath('/accounts');}
...
export async function deleteCar(fd:FormData){const uid=await userId(); const id=String(fd.get('id')); await prisma.car.deleteMany({where:{id,userId:uid}}); revalidatePath('/cars'); revalidatePath('/dashboard');}
...
export async function payBill(fd:FormData){const uid=await userId(); const bill=await prisma.recurringBill.findFirst({where:{id:String(fd.get('id')),userId:uid}}); if(!bill)return; const accountId=String(fd.get('accountId')||bill.accountId); const date=fd.get('date')?new Date(String(fd.get('date'))):new Date(); await assertOpen(uid,date); const desc=String(fd.get('description')||`Bayar tagihan ${bill.name}`); await prisma.$transaction(async tx=>{await tx.recurringBill.update({where:{id:bill.id},data:{status:'PAID',lastPaidAt:date}}); await tx.account.update({where:{id:accountId},data:{balance:{decrement:bill.amount}}}); await tx.transaction.create({data:{userId:uid,accountId,type:'EXPENSE',amount:bill.amount,date,description:desc,sourceType:'bill',sourceId:bill.id}});}); revalidatePath('/bills'); revalidatePath('/dashboard');}
