...
const botCommands: BotCommand[] = [
  ...
  { command: 'accounts', description: 'Daftar akun Anda' }
];
...
async function parseTransaction(text: string, telegramId: number) {
  try {
    ...
    if (!user) return { error: '❌ User tidak ditemukan. Silakan setup di aplikasi terlebih dahulu.' };
    ...
    if (!account) return { error: `❌ Rekening "<b>${accountName}</b>" tidak ditemukan.\n\nKetik /accounts untuk melihat daftar akun.` };
    ...
    return { error: '❌ Error memproses transaksi. Silakan coba lagi.' };
  }
}
...
export async function processTelegramUpdates() {
  ...
  try {
    ...
    for (const update of updates) {
      ...
      if (command === '/start') {
        await handleStartCommand(botToken, chat.id, from.id, from.first_name);
...
export async function registerBotCommands(botToken: string) {
  try {
