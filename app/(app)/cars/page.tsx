  ...
  return (
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_400px]">
            ...
            return (
              <Card key={c.id}>
                <div className="flex justify-between gap-2">
                  <div>
                    <h3 className="text-lg md:text-xl font-black">{c.name}</h3>
                    <p className="text-xs text-slate-400">{c.brand} {c.model} • {c.year} • {c.licensePlate}</p>
                  </div>
                  <span className={c.status === 'SOLD' ? 'text-rose-300' : 'text-emerald-300'}>{c.status}</span>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                  <div><p className="text-slate-400">Modal</p><b>{rupiah(modal)}</b></div>
                  <div><p className="text-slate-400">Est Jual</p><b>{rupiah(c.estimatedSellPrice)}</b></div>
                  <div><p className="text-slate-400">Harga Jual</p><b>{rupiah(c.sellPrice)}</b></div>
                  <div><p className="text-slate-400">Profit/Rugi</p><b>{profit === null ? '-' : rupiah(profit)}</b></div>
                </div>
                {c.debts && c.debts.length > 0 && (
                  <div className="mt-3 rounded-xl bg-rose-400/10 p-3">
                    {c.debts.map(d => (
                      <div key={d.id} className="flex justify-between text-sm">
                        <span className="text-rose-300">{d.name}</span>
                        <span className="font-black">{d.status === 'PAID' ? <span className="text-emerald-300">Lunas</span> : rupiah(d.remainingAmount)}</span>
                      </div>
                    ))}
                  </div>
                )}
                <details className="mt-4">
                  <summary className="cursor-pointer text-emerald-300">Action</summary>
                  <div className="mt-3 space-y-3">
                    <form action={addCarCost} className="grid gap-2">
                      <input type="hidden" name="carId" value={c.id} />
                      <select name="accountId">
                        {accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                      </select>
                      <input name="amount" placeholder="Biaya" />
                      <input name="description" placeholder="Detail biaya" />
                      <input name="date" type="date" defaultValue={todayInput()} />
                      <button className="btn btn-ghost">Tambah Biaya</button>
                    </form>
                    {c.status === 'AVAILABLE' && (
                      <form action={sellCar} className="grid gap-2">
                        <input type="hidden" name="carId" value={c.id} />
                        <select name="accountId">
                          {accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                        </select>
                        <input name="sellPrice" placeholder="Harga jual aktual" />
                        <input name="date" type="date" defaultValue={todayInput()} />
                        <button className="btn btn-primary">Jual Mobil</button>
                      </form>
                    )}
        ...
        <Card>
          <h2 className="font-black mb-4">Tambah Mobil</h2>
          <form action={addCar} className="space-y-3">
            <input name="name" placeholder="Honda CR-V 2.4 AT 2010" required />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <input name="transmission" placeholder="Transmisi" />
              <input name="licensePlate" placeholder="Nopol" />
              <input name="frameNumber" placeholder="No rangka" />
              <input name="engineNumber" placeholder="No mesin" />
              <input name="mileage" placeholder="KM" />
              <input name="purchasePrice" placeholder="Harga beli" />
