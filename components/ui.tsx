...
export function StatCard({ label, value, hint, tone='neutral' }: { label: string; value: number; hint?: string; tone?: 'green'|'red'|'blue'|'purple'|'neutral' }) {
  ...
  return <div className="soft-card rounded-[1.35rem] p-4 md:p-5 min-h-[112px]"><p className="text-[11px] font-black text-slate-400 uppercase tracking-wide">{label}</p><h3 className={cn('mt-3 text-xl md:text-2xl font-black tracking-[-.04em]', tones[tone].split(' ')[0])}>{rupiah(value)}</h3>{hint && <p className="mt-1 text-[11px] text-slate-500">{hint}</p>}</div>;
}
